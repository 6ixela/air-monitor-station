...
void set_ens160_opmode()
{
    ...
    {
        ESP_LOGE(TAG, "Failed to write data to ENS160 opmode");
    }
...
{
    uint8_t data[1];
    esp_err_t res = ens160_read_data(DATA_AQI, data, sizeof(data));
    if (res != ESP_OK)
    {
        ESP_LOGE(TAG, "Failed to read AQI from ENS160: %d", res);
        return 0xFF; // invalid
    }
...
uint16_t get_ens160_tvoc()
{
    ...
    {
        ESP_LOGE(TAG, "Failed to read TVOC from ENS160");
        return 0;
    }
    ...
    if (tvoc <= 600)
    {
        ESP_LOGI(TAG, "TVOC PPB Value: %d  \033[34m[Excellent]\033[0m", tvoc);
    }
    else if (tvoc <= 800)
    {
        ESP_LOGI(TAG, "TVOC PPB Value: %d  \033[32m[Good]\033[0m", tvoc);
    }
    else if (tvoc <= 1000)
    {
        ESP_LOGI(TAG, "TVOC PPB Value: %d \033[33m[Moderate]\033[0m", tvoc);
    }
    else if (tvoc <= 1500)
    {
        ESP_LOGI(TAG, "TVOC PPB Value: %d  \033[38;5;214m[Poor]\033[0m", tvoc);
    }
    else
    {
        ESP_LOGI(TAG, "TVOC PPB Value: %d  \033[31m[Unhealthy]\033[0m", tvoc);
    }
...
uint16_t get_ens160_eco2()
{
    ...
    {
        ESP_LOGE(TAG, "Failed to read eCO2 from ENS160");
        return 0;
    }

    uint16_t eco2 = (data[1] << 8) | data[0];
    ESP_LOGI(TAG, "eCO2 PPM Value: %d", eco2);
    return eco2;
}
...
uint8_t get_ens160_status()
{
    ...
    if (ens160_read_data(0x20, data, sizeof(data)) != ESP_OK)
    {
        ...
        return 4; // Return an error code for failed read
    }
...
void compensate_temp_hum(void)
{
    ...
    float temp_k = aht21_data.temperature + 273.15f; // convert to Kelvin
    int32_t temp_fixed = (int32_t)(temp_k * 64.0f);
    if (temp_fixed < 0)
    {
        temp_fixed = 0;
    }
    if (temp_fixed > 0xFFFF)
    {
        temp_fixed = 0xFFFF;
    }
    uint16_t temp = (uint16_t)temp_fixed;
    uint8_t data[2] = {temp & 0xFF, (uint8_t)(temp >> 8)};

    if (ens160_write_data(0x30, data, sizeof(data)) != ESP_OK)
    {
        ESP_LOGE(TAG, "Failed to write compensated temp/hum to ENS160");
    }
}
